...
test('electron shell keeps renderer isolation and external navigation guarded', async () => {
  ...
  assert.match(mainSource, /const createApiLogStream = \(\) =>/)
  assert.match(mainSource, /Starting API server from Electron/)
  ...
  assert.match(mainSource, /stdio: \['ignore', apiLogStream, apiLogStream\]/)
