...
test('electron shell keeps renderer isolation and external navigation guarded', async () => {
  ...
  assert.match(mainSource, /error\.message\.startsWith\('Refusing to load'\)/)
  assert.match(mainSource, /await waitForAppServer\(apiUrl, 750, desktopServerToken\)/)
  assert.match(mainSource, /if \(error instanceof Error && error\.message\.startsWith\('Refusing to load'\)\) throw error/)
