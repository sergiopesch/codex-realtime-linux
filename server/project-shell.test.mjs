...
test('artifact previews are served through workspace-scoped routes only', async () => {
  ...
  assert.match(appSource, /const artifactPreview =[\s\S]*selectedArtifact &&[\s\S]*selectedArtifact\.workspacePath === selectedWorkspace &&[\s\S]*!artifactMatchesDismissal\(selectedArtifact, dismissedArtifact\)/)
  assert.match(appSource, /workspacePath: artifactPreview\.workspacePath/)
  assert.match(appSource, /const openConversationWindow = \(workspacePath: string, conversationId: string\) => \{[\s\S]*setSelectedArtifact\(null\)/)
  assert.match(appSource, /const toggleWorkspace = \(workspacePath: string\) => \{[\s\S]*setSelectedArtifact\(null\)/)
  assert.match(appSource, /const openSystemScreen = \(screen: SystemScreen\) => \{[\s\S]*setSelectedArtifact\(null\)/)
