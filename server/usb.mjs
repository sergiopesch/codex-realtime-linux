...
export function classifyUsbDevice(properties) {
  ...
  const idPath = normalizeString(properties.ID_PATH || properties.idPath)
  const devpath = normalizeString(properties.DEVPATH || properties.devpath)
  const text = [vendor, model, serial, driver, devname, idPath, devpath].filter(Boolean).join(' ')
