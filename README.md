...
- The server persists this client's local workspace/thread state to `CODEX_REALTIME_STATE_PATH`, defaulting to `~/.local/state/codex-realtime-linux/app-state.json`; overrides must be absolute paths without control characters and within the configured path length cap, the state directory is tightened to `0700`, writes are synced before atomic rename, readable primary state is refreshed into an atomic `.bak` before mutations, corrupted or missing primary state recovers from that backup when available, loaded state is bounded and deduplicated by workspace path and stable row identity, Codex history deletes are stored as workspace-scoped hidden thread IDs rather than broad app-server archive operations, hidden-thread deletes are retained even when a workspace has no saved local chats yet, single-chat deletes send the clicked sidebar row key through the app-state API so legacy duplicate IDs cannot clear the rest of a workspace, Codex history refreshes are additive so partial thread-list responses cannot clear existing workspace chats, and app-state write failures return bounded JSON errors instead of misleading route-specific failures.
