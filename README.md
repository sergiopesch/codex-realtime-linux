...
6. Realtime voice and transcript: start voice, speak, open the transcript, and verify both user and Codex transcript lines appear. Mute, unmute, and stop must work, shorter final transcript events and final response payload fallbacks must not erase text that arrived as deltas, and input transcription failures must appear as bounded transcript errors instead of an empty panel.
