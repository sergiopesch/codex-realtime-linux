...
- Live transcript panel for the current Realtime session; starting voice opens a local transcript thread when none is selected, and transcript lines are saved back into that bound conversation so reopening the thread can show the last voice exchange.
...
The Vite dev server binds to loopback by default and proxies both `/api` and `/workspace-artifacts` to the local API server so generated previews behave the same in dev and in the installed desktop app. The proxy follows the same validated `PORT` override as the API server, defaulting to `3311`. If you explicitly need to expose the renderer on the LAN for device testing, use `npm run dev:lan`.
...
The installer builds the renderer, installs the desktop entry at `~/.local/share/applications/codex-realtime-linux.desktop`, installs hicolor app icons, and writes `scripts/launch-desktop.sh`. If `XDG_DATA_HOME` is set to an absolute path, the desktop entry and icons use that data root; relative paths and paths with control characters are ignored and fall back to `~/.local/share`. The launcher prefers Electron's packaged Linux executable instead of the Node-based Electron shim, and Electron starts the local API server through its own runtime so app-menu launches do not depend on a wide shell `PATH`.

After installation, open the app menu and launch **Codex**. The launcher starts Electron from the packaged Linux binary and, when `script` is available, allocates a small PTY wrapper so no-terminal app-menu launches behave like an attached desktop process; if `script` is unavailable it falls back to a direct Electron exec. Before the `dialout` group handoff and before Electron startup, the launcher resolves the Codex CLI dynamically from `PATH` or npm's configured global prefix and exports an absolute `CODEX_BIN` for the API bridge when possible. Before launch it rebuilds the renderer when `src`, app-owned files in `public`, `index.html`, package, Vite, or TypeScript config files are newer than `dist/index.html`, so double-click launches do not keep serving stale UI after source updates. Generated workspace artifacts under `public/agent-files/` are ignored by this stale-build check so presentations do not become app-shell inputs when this repo is selected as a workspace. Electron starts the local API server and loads the built app from `http://127.0.0.1:3311`; production app-menu launches ignore stale `VITE_DEV_SERVER_URL` values so a leftover dev environment cannot redirect the standalone app away from the built renderer. Linux launches keep the transparent rounded window on a software-rendered path by disabling GPU compositing, rasterization, and accelerated canvas paths before app startup. Electron-managed launches require the local API server to prove it belongs to the current desktop launch. If a previous Electron-managed same-repo server is still on the port, the shell terminates that stale server and starts a fresh one; unrelated or manually started local servers are still refused instead of being reused silently. Launcher failures are written to `~/.local/state/codex-realtime-linux/desktop-launch.log`; each launch writes a timestamped run marker before group handoff checks, so `dialout` re-exec attempts and failures are visible even when the app was opened from the menu. The same log then records the resolved Electron binary before Electron output. API server output from Electron-managed launches is written to `~/.local/state/codex-realtime-linux/api-server.log`, and startup error dialogs include a bounded tail of that API log when available. If `XDG_STATE_HOME` is set to an absolute path, desktop logs use that state root; relative values and paths with control characters are ignored. Desktop logs rotate at 1 MiB with a `.1` backup.

## Generated HTML Previews

Realtime-generated HTML presentations are written into the selected workspace under `public/agent-files/` and shown in the in-app browser preview when the Codex task finishes in the foreground workspace. Realtime voice can open the latest generated preview, open a specific generated relative path, close the browser preview, refresh it, delete the current generated preview, or clear old generated previews through the app's browser-preview tool; it should not tell the user to open or manage generated files outside the desktop app. Codex task prompts also tell agents not to start local HTTP/dev servers, `python http.server`, external browser openers, or file-open commands to verify generated previews; the desktop app owns preview serving through `/workspace-artifacts`, so verification should be done by checking generated files and workspace-relative paths. Closing the browser preview is hide-only and never deletes files. Cleanup is explicit, app-owned, and workspace-scoped: `delete_current` removes only the open indexed artifact, while `clear_old` removes stale indexed artifacts under that workspace's `public/agent-files/` folder while keeping the current preview or newest result. The browser preview is temporary and demand-driven: historical artifacts are indexed in the background, but the app does not reopen old previews on startup, workspace navigation, idle polling, active Codex turns, or system screens. Browser visibility is tied to the current generated-result session and expires after a 3-minute idle viewing window, so closing the preview, switching workspaces, opening Settings/Usage/Profile, starting another task, or leaving the result idle returns the app to the normal voice surface until a new foreground result is ready. Preview leases are not created while a system screen is open or when the completed artifact belongs to a background workspace, and a background artifact completion does not close the current foreground preview. Codex turns are watched for generated-artifact changes during the active turn: changed paths under `public/agent-files/` are mapped back to the indexed top-level `.html` file or artifact-folder `index.html`, so editing an existing generated preview refreshes or opens the in-app browser after Codex completes instead of repeatedly remounting the iframe while files are still being written. Preview iframe loads and self-refreshes do not renew that viewing window; explicit interaction with the preview shell or focusing/hovering inside the iframe can extend it while the user is actively reviewing the result, and regenerated content at the same preview URL remounts the iframe when the artifact timestamp changes. Preview routes are workspace-scoped, use opaque per-process workspace tokens instead of path-encoded tokens, and serve generated files with browser safety headers; the app does not expose a fixed bundled presentation route. The artifact list includes safe artifact folder names whose `index.html` is a real file inside that artifact folder, plus safe top-level generated `.html` files directly under `public/agent-files/`, so common Codex outputs such as `public/agent-files/hello-world.html` can open without a hardcoded viewer route while symlink escapes and malformed preview folders are not surfaced to the UI. Codex task responses expose only the generated artifact's relative path, workspace path, and preview URL; internal absolute artifact write paths remain server-only, and selected-workspace Codex task prompts instruct final answers to use workspace-relative paths instead of absolute local file links. Individual preview files are capped at 25 MiB before being streamed into the app; nested asset requests still require that artifact folder's `index.html` to stay within the same cap, top-level direct previews are limited to `.html`/`.htm`, hidden dotfiles, hidden path segments, or backslash path segments inside artifact folders are not served, and unsupported/binary file extensions are rejected instead of being streamed from the preview route. Previews are intended to be self-contained local artifacts: scripts may run inside the sandboxed iframe, but network/API connections are blocked by preview CSP, the iframe does not get same-origin privileges, the iframe explicitly denies camera, microphone, display capture, geolocation, USB, and serial permissions, same-origin preview/API routes are not allowed to replace the top-level Electron app shell, and Electron IPC is accepted only from the top app frame.
...
The app starts Codex through `codex app-server` by default. Standalone desktop launches now try to resolve Codex from the launch `PATH` or npm's configured global prefix and pass the absolute executable to the API bridge. If your environment needs a different executable, set it explicitly. Blank values and relative path-like overrides are ignored so app-menu launches do not fail later with a confusing spawn error:
...
Malformed absolute path overrides containing control characters are ignored for desktop state roots, installer data roots, and `CODEX_REALTIME_NODE_BIN`.
...
The local API accepts app requests from the desktop origin and Vite dev origins only. To trust another local development origin, add it explicitly. Extra origins must be root `http://localhost`, `http://127.0.0.1`, or `http://[::1]` origins; non-loopback URLs, paths, credentials, query strings, hashes, overlong entries, duplicate entries, and entries beyond the bounded allowlist are ignored. Allowed browser origins receive CORS response and preflight headers for `/api/*`; untrusted origins are rejected before they reach state, Codex, vision, weather, or hardware routes:
...
Relative, overlong, or control-character state paths are ignored and fall back to the default XDG state location.
...
Relative, overlong, or control-character secret paths are ignored and fall back to the default XDG config location.
...
- `/api/usb/events` reports Linux USB serial add/remove events with bounded device metadata and bounded monitor status errors, and flags Arduino-like devices. Manual USB scans use `POST /api/usb/events/scan` so stateful probing is not hidden behind a GET request.
...
- `/api/codex/task` requires an explicit existing workspace `cwd`; Realtime voice routing only accepts the workspace currently selected in the app, app-source workspaces are rejected unless `CODEX_ALLOW_APP_SOURCE_TASKS=true`, and allowed app-source tasks still receive protected-path guardrails for the repo root, subdirectories, and symlinked app workspaces.
...
- Mutating `/api/*` routes reject untrusted browser origins, and routes with JSON payloads reject form-style, malformed, non-object, or oversized requests before they can touch state, Settings, vision, weather, Codex, or Arduino hardware.
- The built app shell is served with browser safety headers: a CSP limited to app assets, local API calls, and OpenAI Realtime calls; no-sniff and no-referrer headers; anti-framing headers; and permissions policy that allows microphone/display capture only for the app itself. Electron also denies permission checks and requests except media/display capture from the trusted top app frame, and blocks top-level navigations or redirects away from the local app shell. Generated artifact previews use a stricter separate CSP that blocks network/API connections.
- The server persists this client's local workspace/thread state to `CODEX_REALTIME_STATE_PATH`, defaulting to `~/.local/state/codex-realtime-linux/app-state.json`; overrides must be absolute paths without control characters and within the configured path length cap, the state directory is tightened to `0700`, writes are synced before atomic rename, readable primary state is refreshed into an atomic `.bak` before mutations, corrupted or missing primary state recovers from that backup when available, loaded state is bounded and deduplicated by workspace path and stable row identity, Codex history deletes are stored as workspace-scoped hidden thread IDs rather than broad app-server archive operations, hidden-thread deletes are retained even when a workspace has no saved local chats yet, sidebar selection, voice transcript saves, Codex status updates, and single-chat deletes use strict row keys so legacy duplicate IDs cannot open, mutate, remove the wrong workspace thread, or fall back to a broader ID delete when the UI has stale row state, Codex history refreshes are additive so partial thread-list responses cannot clear existing workspace chats, and app-state write failures return bounded JSON errors instead of misleading route-specific failures.
- The server persists Settings-saved API secrets to `CODEX_REALTIME_SECRETS_PATH`, defaulting to `~/.config/codex-realtime-linux/secrets.json`; overrides must be absolute paths without control characters and within the configured path length cap, the secrets directory is tightened to `0700`, writes are synced before atomic rename, malformed or oversized saved keys are ignored on load, and Settings responses do not expose the absolute secrets file path.
...
Use `npm run verify:live` to write machine-observable desktop evidence to `docs/mvp-live-probe-result.md`.
...
9. USB detection: with voice running, connect the board and run `curl -s -X POST -H 'Content-Type: application/json' --data '{}' http://127.0.0.1:3311/api/usb/events/scan`. The app should briefly acknowledge the detected board without pretending to read sketch or serial data.
...
For a non-invasive live environment probe of the running desktop session, run:

```bash
npm run verify:live
```

The live probe records service state, desktop files, logs, API health, Realtime token endpoint status without storing token contents, media-device enumeration, USB/Arduino visibility, and weather route behavior. It writes `docs/mvp-live-probe-result.md` and complements, but does not replace, the human checklist.
...
```bash
curl -s -X POST -H 'Content-Type: application/json' --data '{}' http://127.0.0.1:3311/api/usb/events/scan
```
...
With voice running and the board connected, say something like: “turn on the Arduino light” or “make the Arduino LED blink”. Realtime treats these as direct hardware upload requests, not Codex coding tasks: it can call `arduino_status` to report `arduino-cli`, board, and serial-port detection, then calls `arduino_upload_sketch` with an explicit supported action. The server compiles the sketch, then uploads it to the detected board. Direct upload API calls must also include `action` as `onboard_led_on`, `onboard_led_blink`, or `custom_sketch`; missing actions fail before compile/upload so incomplete requests cannot flash board firmware. For a single detected board, the server prefers a stable `/dev/serial/by-id/*` link when available, falling back to `/dev/ttyACM*` or `/dev/ttyUSB*`. If an upload through a stable by-id alias fails but the matching tty target is known, the server retries upload once on that tty target after a successful compile and reports the port actually used. If multiple boards are detected, or if multiple serial ports exist and board metadata cannot identify one target, upload fails until a supported `/dev/ttyACM*`, `/dev/ttyUSB*`, or `/dev/serial/by-id/*` port is supplied explicitly, so the app does not guess and flash the wrong board. Explicit upload ports must be currently detected; stale or arbitrary serial paths fail before compile/upload. When an explicit stable by-id port maps back to a detected tty board, including when more than one tty device is connected, the app uses that board's detected FQBN for compile/upload instead of falling back to the default Uno target. If `arduino-cli` reports multiple candidate board matches for a port, the server prefers a valid FQBN candidate before falling back to the default target.
